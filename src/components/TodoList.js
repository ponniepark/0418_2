...
const TodoList = () => {
  ...
  const addTodo = async () => {
    ...
    setTodos([...todos, { id: docRef.id, text: input, completed: false, date }]);
    setInput("");
  ...
  const toggleTodo = (id) => {
    ...
    const newTodos = todos.map((todo) => {
      if (todo.id === id) {
        ...
        updateDoc(todoDoc, { completed: !todo.completed, date : !todo.date,  });
        // ...todo => id: 1, text: "할일1", completed: false
        return { ...todo, completed: !todo.completed, date : !todo.date};
      } else {
        return todo;
      }
    });
